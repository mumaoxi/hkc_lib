...
import com.hkc.constant.CommonDefine.UmengEvent;
...
public class HKCSplashActivity extends Activity implements VoListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		SharedPreferences sp = getSharedPreferences(CommonDefine.APP_PREF_FILE,
				MODE_PRIVATE);
		Editor editor = sp.edit();
		editor.putInt(CommonDefine.PrefKey.SCREEN_WIDTH, outMetrics.widthPixels);
		editor.putInt(CommonDefine.PrefKey.SCREEN_HEIGHT,
		 ...
		 * 添加HKC友盟统计
		 */
		UmengClickAgent.onData(context);
		...
		// 统计多少用户手机安装了微信
		if (HelperUtils.isInstalledWx(context)
				&& !HelperUtils.isUploadedInstalledWx(context)) {
			UmengClickAgent.onEvent(context, UmengEvent.WEIXIN_INSTALLED);
			HelperUtils.setUploadedInstalledWx(context, true);
		}

		// 统计多少用户手机安装了微信
		if (HelperUtils.isInstalledQQ(context)
				&& !HelperUtils.isUploadedInstalledQQ(context)) {
			UmengClickAgent.onEvent(context, UmengEvent.QQ_INSTALLED);
			HelperUtils.setUploadedInstalledQQ(context, true);
		}
