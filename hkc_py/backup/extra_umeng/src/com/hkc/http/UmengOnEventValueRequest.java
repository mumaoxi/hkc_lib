...
public class UmengOnEventValueRequest extends
		FreshRequest<UmengOnEventValueResponse> {

	private Context context;
	private Map<String, String> tagLabel = new HashMap<String, String>();
	...
	public String getRequestBody() {
		...
		try {
			...
			for (Entry<String, String> entry : tagLabel.entrySet()) {
				JSONObject eventObject = new JSONObject();
				eventObject.put("date",
						DateUtils.formatDate(dateNow, "yyyy-MM-dd"));
				eventObject.put("session_id",
						MD5.encrypt(device_id + System.currentTimeMillis())
								.toUpperCase(Locale.CHINA));
				eventObject.put("time",
						DateUtils.formatDate(dateNow, "HH:mm:ss"));
				eventObject.put("acc", 1);
				eventObject.put("tag", entry.getKey());
				eventObject.put("label", entry.getValue());
				eventsArray.put(eventObject);
			}
			bodyObject.put("event", eventsArray);
	...
	public void setTagLabel(Map<String, String> tagLabel) {
